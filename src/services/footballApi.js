...
// Track recently used players to avoid repetition
let recentlyUsedIds = [];

// Fetch random players from top leagues
const fetchRandomPlayersFromLeague = async () => {
  ...
  try {
    ...
    if (data.response && data.response.length > 0) {
      // Filter out recently used and players without photos
...
export const fetchTwoRandomPlayers = async () => {
  ...
  // Shuffle and pick 2 random players
  ...
  console.log('Player 1 image:', principal.image);
  console.log('Player 2 image:', secondary.image);
